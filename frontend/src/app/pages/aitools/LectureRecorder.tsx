...
export default function LectureRecorder() {
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        const audioFile = new File([audioBlob], 'lecture_recording.webm', { type: 'audio/webm' });
        setFile(audioFile);
  ...
  return (
    <div className="space-y-6 pb-10">
      <Link to="/app/tools" className="flex items-center gap-2 text-text-secondary hover:text-text-primary mb-4 w-fit transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6 bg-surface border border-border-default h-fit">
          <div className="flex flex-col items-center justify-center bg-primary-bg rounded-2xl py-10 mb-6 border-2 border-dashed border-border-default h-[250px]">
             ...
             {!isRecording ? (
               <button type="button" onClick={handleRecordingToggle} className="w-20 h-20 bg-rose/10 hover:bg-rose/20 text-rose rounded-full flex items-center justify-center transition-all hover:scale-105 active:scale-95 shadow-[0_0_30px_rgba(244,63,94,0.1)]">
                 <Mic className="w-8 h-8" />
               </button>
             ) : (
               <button type="button" onClick={handleRecordingToggle} className="w-20 h-20 bg-rose text-white rounded-xl flex items-center justify-center transition-all hover:scale-105 active:scale-95 animate-pulse shadow-[0_0_40px_rgba(244,63,94,0.4)]">
                 <StopCircle className="w-10 h-10" />
               </button>
             )}
             
             <div className="mt-6 font-mono text-xl font-bold text-text-primary tracking-widest">{formatTime(recordingTime)}</div>
             <p className="text-text-tertiary text-sm mt-2 font-medium">{isRecording ? "Recording in progress..." : "Tap to start recording"}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
               ...
               {file && file.name === 'lecture_recording.webm' ? (
                 <div className="w-full bg-primary-bg rounded-lg p-3 text-emerald border border-emerald/30 font-medium flex items-center justify-between">
                   <div className="flex items-center gap-2">
                     <CheckCircle2 className="w-5 h-5" />
                     <span>Recording saved ({formatTime(recordingTime)})</span>
                   </div>
                   <button type="button" onClick={() => { setFile(null); setRecordingTime(0); }} className="text-rose text-sm font-bold hover:underline">Discard</button>
                 </div>
               ) : (
                 <input 
                    type="file" 
